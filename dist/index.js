...
const SENT_EMAILS_FILE = path_1.default.join(__dirname, 'sentEmails.json');
// Загружаем список уже отправленных email'ов
const loadSentEmails = () => {
    if (fs_1.default.existsSync(SENT_EMAILS_FILE)) {
        return JSON.parse(fs_1.default.readFileSync(SENT_EMAILS_FILE, 'utf8'));
    }
    return [];
};
// Сохраняем обновленный список отправленных email'ов
const saveSentEmails = (sentEmails) => {
    fs_1.default.writeFileSync(SENT_EMAILS_FILE, JSON.stringify(sentEmails, null, 2), 'utf8');
};
